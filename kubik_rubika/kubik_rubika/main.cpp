...
int main() {
    ...
    Cube cube;  // Создаем новый кубик рубика
    cube.print();  // Печатаем начальное состояние кубика
    ...
    while (true) {
        ...
        // Поворот кубика в зависимости от введенной команды
        switch (move) {
        case 'U': case 'u':
            cube.rotate_face(0, true);  // Вращение верхней стороны по часовой стрелке
            break;
        case 'D': case 'd':
            cube.rotate_face(1, true);  // Вращение нижней стороны по часовой стрелке
            break;
        case 'L': case 'l':
            cube.rotate_face(2, true);  // Вращение левой стороны по часовой стрелке
            break;
        case 'R': case 'r':
            cube.rotate_face(3, true);  // Вращение правой стороны по часовой стрелке
            break;
        case 'F': case 'f':
            cube.rotate_face(4, true);  // Вращение передней стороны по часовой стрелке
            break;
        case 'B': case 'b':
            cube.rotate_face(5, true);  // Вращение задней стороны по часовой стрелке
            break;
        default:
            std::cout << "Enter a move:" << std::endl << "Rotate the top side clockwise - U" << std::endl;
            std::cout << "Rotate bottom side clockwise - D" << std::endl;
        ...
        cube.print();  // Печать состояния кубика после каждого хода
